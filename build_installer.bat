...
echo.
echo [3/5] Checking runtime dependencies for full package...
%PYTHON_CMD% -c "import sys, importlib.util; from pathlib import Path; vendor=Path('vendor').resolve(); sys.path.insert(0, str(vendor)); [sys.path.insert(0, str((vendor / line.strip()).resolve())) for p in vendor.glob('*.pth') for line in p.read_text(encoding='utf-8').splitlines() if line.strip() and not line.strip().startswith('#')]; mods=['cv2','mss','pynput','win32gui','yaml','pyautogui','numpy','torch','ultralytics','paddle','paddleocr']; missing=[m for m in mods if importlib.util.find_spec(m) is None]; print('missing:', ', '.join(missing) or 'none'); raise SystemExit(1 if missing else 0)"
if errorlevel 1 goto :runtime_deps_error

echo.
echo [4/5] Building onedir app with PyInstaller...
set "PACKAGE_ML=1"
...
echo Installer: dist\installer\姊﹀够瑙嗚杈呭姪_Setup.exe
exit /b 0
