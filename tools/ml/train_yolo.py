...
用法:
  ...
  # 继续训练已有模型
  python tools/ml/train_yolo.py --base tools/ml/runs/detect/mhxy_train/weights/last.pt --epochs 50

  # 验证模型
  python tools/ml/train_yolo.py --val --base tools/ml/runs/detect/mhxy_train/weights/best.pt

数据集准备:
  1. 采集游戏截图放到 tools/ml/images/train/
  2. 用标注工具（labelImg / CVAT）标注后导出 YOLO 格式
  3. 标注文件(.txt)放到 tools/ml/labels/train/
...
def train(base_model: str, epochs: int, batch: int, imgsz: int, name: str, device: str):
    ...
    print(f"\n训练完成！最佳模型: {best_pt}")
    print(f"训练产物: {best_pt}")
    print("请先导出 ONNX，再将 config.yaml 中 yolo.model_path 指向 models/yolo/best.onnx。")
    return best_pt
...
def main():
    ...
    parser.add_argument("--device", default="cpu", help="训练设备 (cuda / cpu / 0)")
